...
class Turtlebot3ObstacleDetection(Node):
    ...
    def detect_obstacle(self):
        ...
        obstacle_distance_left = min(self.scan_ranges[-90:-54])
        obstacle_distance_front_left = min(self.scan_ranges[-54:-18])

        #front interval
        #-36 to -1 range used as the robot has a hard time with transition from positive-negative angle range
        obstacle_distance_front_1 = min(self.scan_ranges[-18:-1])
        obstacle_distance_front_2 = min(self.scan_ranges[0:18])
        obstacle_distance_front = min(obstacle_distance_front_1, obstacle_distance_front_2)

        obstacle_distance_front_right = min(self.scan_ranges[18:54])
        obstacle_distance_right = min(self.scan_ranges[54:90])
       ...
       #detects obstacle in front
        if obstacle_distance_front < 0.29:
       ...
       #############################################################################################################
        ...
        collision_detected = (obstacle_distance_front < 0.18 or obstacle_distance_left < 0.15 or obstacle_distance_front_left < 0.15 or obstacle_distance_right < 0.15 or obstacle_distance_front_right < 0.15)
